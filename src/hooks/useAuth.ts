...
export function useDashboard() {
  return useQuery({
    queryKey: ["plus-dashboard"],
    queryFn: async () => {
      const res = await fetch("/api/plus/dashboard");
      const payload = await res.json().catch(() => null);
      if (!res.ok) throw new Error(payload?.message || "خطا در دریافت داشبورد");
      return payload.data;
    },
  });
}

export function usePlusList(path: string, enabled = true) {
  return useQuery({
    queryKey: ["plus", path],
    enabled,
    queryFn: async () => {
      const res = await fetch(`/api/plus/${path}`);
      const payload = await res.json().catch(() => null);
      if (!res.ok) throw new Error(payload?.message || "خطا در دریافت اطلاعات");
      return payload;
    },
  });
}
